...
public class Q1 {
	public static void main(String[] args) throws IOException {
	    ...
	    System.out.print("請輸入年齡(6~15):> ");
	    int user_old=sc.nextInt();
	    System.out.print("請輸入性別(男性輸入1/女性輸入2):> ");
...
//	    }
	    ...
	    System.out.printf("97~109學年度%d歲", user_old);
	    if(user_gender==1) { //性別為男生
	    	System.out.print("男性的身高資料:> ");
	    	for(int x=0;x<k;x++) {
	    		for(int y=2;y<5;y++){
	    			if(z<13) {
	    ...
	    //輸出簡單迴歸結果
	    System.out.println("=========身高預測=========");
	    System.out.printf("迴歸方程式:> 身高=%f*學年度+%f\n", beta, alpha);
	    if(user_gender==1)
	    	System.out.printf("預測110年度男性%d歲身高:> %f\n", user_old, predict_height);
	    else
	    	System.out.printf("預測110年度女性%d歲身高:> %f\n", user_old, predict_height);
	    System.out.printf("R平方:> %f\n", r2);
	    
	    //H0:β等於0;H1:β不等於0。雙尾，α=0.05
	    //計算F-test
	    System.out.println("=========計算F-test(α=0.05)=========");
	    ...
	    if(f>(1/f_left) && f<f_right) { //F值介於左尾和右尾，不拒絕虛無假說
	    	System.out.println("此迴歸模型無預測能力");
	    }	
	    else { //F值小於左尾或大於右尾，拒絕虛無假說
	    	System.out.println("此迴歸模型有預測能力");
	    }
	    
	    //計算p-value
	    System.out.println("=========計算p-value(α=0.05)=========");
